...
async function draftAll() {
  ...
  const toDraft = uis.filter((ui) => (ui.textarea.value || "").trim().length === 0);

  if (toDraft.length === 0) {
    setTopStatus("All drafts are already prepared. You can edit or send them now.");
    return;
  }
  ...
  setTopStatus(`Requesting drafts for ${toDraft.length} message(s)...`);

  let done = 0;
  for (const ui of toDraft) {
    await handleDraft(ui);
    done += 1;
    setTopStatus(`Drafted ${done} of ${toDraft.length}`);

    // Small pause to avoid rapid-fire API calls.
    await sleep(750);
  }

  setTopStatus(`Finished drafting ${toDraft.length} message(s).`);
  btnDraftAll.disabled = false;
  btnSendAll.disabled = false;
  btnLoad.disabled = false;
}
